 ...
 */
public abstract class Command<T, S extends CrudService<?>> {
     ...
     */
    public <T> void validate(T value) throws CommandException {
     ...
     */
    public <T> void validate(T value, Class<?>... groups) throws CommandException {
     ...
     */
    public <T> T readBody(HttpServletRequest request, Class<T> tClass) throws CommandException {
        String body = null;
        try {
     ...
     */
    public abstract T execute(HttpServletRequest request, S service) throws CommandException;

    protected long extractIdParameter(HttpServletRequest request) throws CommandException {
        try {
            long id = Long.parseLong(request.getParameter(RequestParameter.ID));
            if (id < 1) throw new CommandException("id should be positive number", CustomError.BAD_REQUEST);
            return id;
        } catch (NumberFormatException e) {
            throw new CommandException("id format is incorrect", CustomError.BAD_REQUEST, e);
        }
